...
def create_app() -> FastAPI:
    ...
    _OPENAPI_TAGS = [
        ...
        {"name": "Growth", "description": "Autonomous marketing engine — content, SEO, case studies, A/B testing."},
        {"name": "Partners", "description": "Partner portal — registration, referrals, commissions, white-label."},
        {"name": "Payments", "description": "Payment processing — Moyasar, STC Pay, SADAD, subscriptions."},
        {"name": "Commercial Ops", "description": "Commercial operations — auto-graduation, expansion, upsell, contracts."},
        {"name": "Metrics", "description": "Prometheus metrics, LLM cost tracking, SLA monitoring."},
    ...
    # ── Wave 3: Distribution Engines ─────────────────────────────────
    try:
        from autonomous_growth.api import router as growth_router
        app.include_router(growth_router, prefix="/api/v1/growth")
    except Exception as _g_exc:
        import logging as _logging
        _logging.getLogger(__name__).warning("growth_router_skipped: %s", _g_exc)

    try:
        from integrations.partner_portal.api import router as partner_router
        app.include_router(partner_router, prefix="/api/v1/partners")
    except Exception as _p_exc:
        import logging as _logging
        _logging.getLogger(__name__).warning("partner_router_skipped: %s", _p_exc)

    # ── Wave 3: Payments & Recurring Revenue ────────────────────────
    try:
        ...
        import logging as _logging
        _logging.getLogger(__name__).warning("payments_router_skipped: %s", _pay_exc)

    # ── Wave 3: Commercial Ops (Auto-graduation, Expansion, Upsell) ─
    try:
        from api.routers import commercial_ops_router
        app.include_router(commercial_ops_router.router, prefix="/api/v1")
    except Exception as _co_exc:
        import logging as _logging
        _logging.getLogger(__name__).warning("commercial_ops_router_skipped: %s", _co_exc)

    # ── Wave 4: Metrics (Prometheus) ────────────────────────────────
    try:
        from api.middleware.metrics import metrics_router
        app.include_router(metrics_router)
    except Exception as _m_exc:
        import logging as _logging
        _logging.getLogger(__name__).warning("metrics_router_skipped: %s", _m_exc)
    ...
    async def root() -> dict[str, object]:
        return {
            ...
            # Wave 3 — Distribution Engines
            "growth_content_queue": "/api/v1/growth/content/queue",
            "growth_seo_clusters": "/api/v1/growth/seo/clusters",
            "growth_case_studies": "/api/v1/growth/case-studies",
            "partners_registration": "/api/v1/partners/register",
            "partners_referrals": "/api/v1/partners/referrals",

            # Wave 3 — Payments & Commercial Ops
            "payments_subscriptions": "/api/v1/payments/subscription",
            "payments_stc_pay": "/api/v1/payments/stc-pay",
            "commercial_auto_graduate": "/api/v1/commercial/auto-graduate",
            "commercial_expansion": "/api/v1/commercial/expansion/opportunities",
            "commercial_upsell": "/api/v1/commercial/upsell/auto",

            # Wave 4 — Metrics & Intelligence
            "metrics": "/metrics",
            "intelligence_capital_allocator": "/api/v1/intelligence/capital-allocator",
        }
