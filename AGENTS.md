...
- Prefer the smallest reversible change that advances the current verified objective.
- Prefer fixing or clarifying existing code over adding parallel systems or duplicate features.
- Run focused tests and verification automatically for safe L0–L4 work when they are needed to establish evidence; use exact-head isolated worktrees and respect resource budgets.
- Do not perform L5 deployment, secret, provider, billing, production-data, external-send, publish, spend, contract, tender, or protected-main effects without the required exact action-bound authority.
...
- **Official Railway launch:** `bash scripts/railway_prod_bootstrap.sh` (legacy HOLD-only; no DDL/seed/API mutation) · `bash scripts/official_launch_verify.sh` → `OFFICIAL_LAUNCH_VERDICT=PASS` · env matrix: `python3 scripts/railway_launch_env_check.py` · full A–D: `bash scripts/launch_execution_railway.sh`
...
Secrets: never print/read .env, keys, tokens; evidence states PRESENT/MISSING/ROTATION_REQUIRED, never values. NEVER ASK FOUNDER TO PASTE SECRET INTO CHAT — use hidden TTY flow `scripts/ops/repair_dealix_openclaw_telegram.sh:1` to `~/.config/dealix-secrets/openclaw-telegram.token:1` 0600. Ollama stays loopback-only and bounded. External sends stay draft-until-approved. Money ledgers stay separate (creator/sponsorship ≠ Dealix revenue; proposal/tender ≠ revenue).
