"""Manual research connector — accepts founder-supplied structured notes.
...
class ManualResearchConnector(BaseConnector):
    manifest = ConnectorManifest(
        name="Manual research (founder)",
        source_type="manual_research",
        allowed_use="Founder reads public web/social and writes a structured note",
        restricted_use=["Behind-login data", "Private content"],
        risk_level="low",
        notes="Highest trust. Slow but premium.",
    )
    ...
    def normalize(self, raw: list[dict]) -> list[dict]:
        return [normalize_lead(r, "manual_research", str(self._source_note)) for r in raw]
