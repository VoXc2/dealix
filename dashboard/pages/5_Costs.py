...
with col1:
    window = st.selectbox("النافذة الزمنية", [1, 6, 24, 72, 168, 720], index=2,
                           format_func=lambda h: f"{h} ساعة")
with col2:
    group_by = st.selectbox("تجميع حسب", ["model", "provider", "task"])

try:
    r = httpx.get(f"{API}/api/v1/admin/costs?window_hours={window}&group_by={group_by}",
