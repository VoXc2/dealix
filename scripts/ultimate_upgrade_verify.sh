...
# Strict raw sweep of the customer-app surfaces only — these pages must
# carry zero forbidden tokens. Marketing pages (index/launchpad) legitimately
# use negation copy ("بدون cold WhatsApp ولا scraping") and are covered by
# the allowlist-aware FORBIDDEN_CLAIMS check above. Comments and
# <script>/<style> blocks are stripped so the standard "not guaranteed
# outcomes" disclaimer does not register as a positive claim.
