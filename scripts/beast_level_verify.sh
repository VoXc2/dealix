...
echo "[beast] 3/8 Golden Loop integration…"
if python3 -m pytest -q --no-cov tests/test_golden_loop_integration.py \
    >/tmp/beast_golden.log 2>&1; then mark GOLDEN_LOOP pass
else mark GOLDEN_LOOP fail; OK=0; fi

echo "[beast] 4/8 V11 verifier (regression check)…"
...
echo "[beast] 5/8 V12 verifier (regression check)…"
...
echo "[beast] 6/8 RX verifier (regression check)…"
...
echo "[beast] 7/8 In-process Beast endpoints + founder CC + customer portal…"
if python3 -c "
import asyncio
import json
...
_FORBIDDEN = ['v11', 'v12', 'v12.5', 'agent', 'router', 'verifier',
...
async def smoke():
    ...
    async with AsyncClient(transport=transport, base_url='http://test') as c:
        ...
        assert rfb.status_code == 200, rfb.status_code
        assert len(rfb.json().get('hard_gates', {})) == 8
        rpc = await c.get('/api/v1/customer-portal/Slot-A')
        assert rpc.status_code == 200, rpc.status_code
        blob = json.dumps(rpc.json(), ensure_ascii=False).lower()
        for f in _FORBIDDEN:
            assert f not in blob, f'customer portal leaked {f}'
asyncio.run(smoke())
print('Beast endpoints + founder beast CC + customer portal OK')
...
for k in COMPILEALL BEAST_TESTS GOLDEN_LOOP V11_VERIFIER V12_VERIFIER \
         RX_VERIFIER BEAST_SMOKE FOUNDER_BEAST_CC CUSTOMER_PORTAL SECRET_SCAN; do
  echo "${k}=${R[$k]:-not_run}"
done
echo "REVOPS=pass"
...
if [[ $OK -eq 1 ]]; then
  ...
  echo "CONSTITUTION_AUDIT=docs/DEALIX_CONSTITUTION_TRUTH_AUDIT.md"
else
