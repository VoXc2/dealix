...
# The verifier above guarantees a repository-local runtime with the declared
# application/test dependencies. Reuse it for every subsequent weekly step.
PY="${DEALIX_AUTOMATION_PYTHON:-$ROOT/.venv/bin/python}"
if [[ ! -x "$PY" ]]; then
  echo "FOUNDER_WEEKLY_LOOP=FAIL"
  echo "automation python unavailable after founder verification: $PY"
  exit 1
fi
