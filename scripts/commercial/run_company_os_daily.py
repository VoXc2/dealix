...
the Meta-Operating System V2 Control Kernel first, then existing company law,
materializes the real website-diagnostic intake bridge from the existing
Revenue Ops store, delegates one-for-one to ``run_self_operating_company_os.py``,
then runs the bounded Strategy Execution Orchestrator. No layer owns a parallel
state store, authority model, target source, scheduler, CRM, approval system,
proof ledger, or permanent agent fleet.
"""
