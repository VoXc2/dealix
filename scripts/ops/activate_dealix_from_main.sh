...
if [[ -x "$HERMES" ]]; then
  ...
  OUT="$(timeout 300 sudo -iu "$RUN_USER" bash -lc 'export PATH="$HOME/.local/bin:$HOME/.hermes/bin:$PATH"; cd /opt/dealix/workspace/dealix; hermes chat -Q --ignore-rules --toolsets clarify --max-turns 1 -q "Do not use tools. Reply with exactly: DEALIX_HERMES_8K_OK"' 2>&1 || true)"
