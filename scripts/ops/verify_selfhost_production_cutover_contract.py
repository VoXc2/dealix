...
required = {
    ...
    "cutover": ["--preflight", "--stage", "--cutover", "--verify-public", "DEALIX_STAGE_PRODUCTION", "DEALIX_PUBLIC_CUTOVER", "DEALIX_TLS_BOOTSTRAP_STRATEGY", "managed_cert_present", "public_acme_after_dns", "ORIGIN_TLS_CERT_STORAGE=", "VALID_HOST_CERTS_PRESENT", "ORIGIN_TLS_REQUIRED_HOSTS", "openssl x509 -noout -checkend 86400 -checkhost", "TLS_BOOTSTRAP_REQUIRED=", "PUBLIC_TLS_EXACT_SHA=PASS", "ROLLBACK_REQUIRED=YES", "ROLLBACK_ACTION=RESTORE_PREVIOUS_DNS_ORIGIN", "POST_DNS_PUBLIC_VERIFY=REQUIRED", "ROLLBACK_ON_TLS_OR_SHA_FAILURE=RESTORE_PREVIOUS_DNS_ORIGIN", "DEALIX_DATA_MIGRATION_RECEIPT", "DEALIX_SOURCE_BACKUP_RECEIPT", "DEALIX_SELFHOST_BACKUP_RECEIPT", "DEALIX_QUIESCENCE_RECEIPT", "verify_selfhost_cutover_receipts.py", "CONFIRM_SHA", "0.0.0.0:80", "0.0.0.0:443", "DNS_MUTATION=NOT_EXECUTED", "RAILWAY_DECOMMISSION=NOT_EXECUTED", "PRODUCTION_GREEN=NOT_PROVEN"],
