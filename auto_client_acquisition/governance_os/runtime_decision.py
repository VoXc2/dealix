"""Runtime governance decision — maps checks to the compliance
``GovernanceDecision`` vocabulary, and provides ``decide`` as the
single-authority gate for any action.
"""
...
_PASSPORT_REQUIRED_ACTIONS = frozenset(
    {"run_scoring", "generate_draft", "export_outreach", "analyze_records", "score_accounts"}
)


def decide(*, action: str, context: dict[str, Any] | None = None) -> DecisionResult:
    """Single-authority governance decision for an action.

    Context keys (all optional): ``source_passport``, ``intended_use``,
    ``channel``, ``mode``, ``is_cold``, ``explicit_consent``,
    ``contains_pii``, ``external_use``, ``text``.
    """
    # Lazy import: avoids an import-time cycle through the data_os package.
    ...
    # 2. Hard-forbidden channel x mode combinations.
    ...
    # 3. WhatsApp cold-without-consent.
    if channel == "whatsapp" and ctx.get("is_cold") and not ctx.get("explicit_consent"):
        return DecisionResult(
            decision=GovernanceDecision.BLOCK,
    ...
    if action in _PASSPORT_REQUIRED_ACTIONS or action.startswith(
        ("score_", "draft_", "generate_")
    ):
        if passport is None:
            return DecisionResult(
                decision=GovernanceDecision.BLOCK,
    ...
    return DecisionResult(
        decision=decision, reasons=tuple(reasons), safe_alternative=safe_alt
    )
