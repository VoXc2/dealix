...
Re-exports the canonical ``ValueLedgerEvent`` / ``value_ledger_event_valid``
from ``proof_architecture_os.value_ledger`` for cross-package callers.
"""
...
def summarize(*, customer_id: str, period_days: int = 30) -> dict[str, Any]:
    ...
    by_tier: dict[str, dict[str, float]] = {
        t: {"count": 0, "total_amount": 0.0} for t in ALLOWED_TIERS
    }
    for ev in in_window:
