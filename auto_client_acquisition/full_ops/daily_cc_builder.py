"""Compose Full-Ops daily command center payload (shared by API routers)."""
...
from auto_client_acquisition.full_ops import get_default_queue, prioritize
...
def hard_gates_dict() -> dict[str, bool]:
    return dict(_HARD_GATES)


def _safe(name: str, fn, default: Any, degraded: list[str]) -> Any:
    try:
        return fn()
...
def compose_daily_command_center() -> dict[str, Any]:
    """Full snapshot for founder daily ops (same payload as GET daily-command-center)."""
    degraded: list[str] = []
...
__all__ = ["compose_daily_command_center", "hard_gates_dict"]
