...
Two APIs coexist here:
  - Lightweight row metrics for account-style dicts (``*_completeness``,
    ``duplicate_ratio_by_field``, ``summarize_table_quality``).
  - The 0-100 composite ``compute_dq`` / ``DQScore`` used by the Data Pack
    offer and the delivery sprint.
"""
...
if TYPE_CHECKING:  # imported lazily — avoids a cycle via revenue_data_intake
...
# Phase 1 stated weights: 60% completeness + 40% inverse-duplicate, plus
# two small modifiers — format_consistency and source_clarity.
@dataclass
class DQScore:
    overall: float
    completeness: float
