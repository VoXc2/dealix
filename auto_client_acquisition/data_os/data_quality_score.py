...
Two coexisting APIs:
- row/table metrics (``account_row_completeness``, ``mean_completeness``,
  ``summarize_table_quality``) — the canonical lightweight surface.
- the 0-100 composite ``DQScore`` / ``compute_dq`` — consumed by the
  Data OS router and the delivery sprint.
"""
...
# Weights: completeness 0.60, duplicate_inverse 0.30, format 0.05,
# source_clarity 0.05.
...
def compute_dq(
    *,
    ...
    """Weighted 0-100 score: completeness 0.60, duplicate_inverse 0.30,
    format 0.05, source_clarity 0.05."""
    ...
    overall = round(
        completeness * 0.60 + dup_inv * 0.30 + fmt * 0.05 + src * 0.05, 2
    )
    return DQScore(
        overall=overall,
        ...
        breakdown={
            ...
            "missing_pct_mean": round(
                sum(preview.missing_pct.values())
                / max(1, len(preview.missing_pct)),
                2,
            ),
            "weights": {
                "completeness": 0.60,
