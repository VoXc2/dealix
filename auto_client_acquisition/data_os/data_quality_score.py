...
Also exposes the ``DQScore`` composite (0–100 over completeness + dedupe +
format consistency + source clarity) consumed by the Data OS router and the
delivery factory.
"""
...
@dataclass
class DQScore:
    """Composite data-quality score (0–100) with a transparent breakdown."""
...
def _format_consistency(preview: ImportPreview) -> float:
    # Heuristic: a column with partial-missing values AND a format-sensitive
    # name costs 5 points; capped at -30.
    penalty = 0.0
...
def compute_dq(
    *,
    ...
    """Weighted composite: completeness 0.60, duplicate_inverse 0.30,
    ...
    overall = round(
        completeness * 0.60 + dup_inv * 0.30 + fmt * 0.05 + src * 0.05,
        2,
    )
    return DQScore(
        overall=overall,
