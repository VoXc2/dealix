...
``import_preview_csv`` delegates to revenue_data_intake (the single source of
truth for CSV parsing). ``preview`` / ``ImportPreview`` provide the
client-facing, PII-aware column summary consumed by the data_os router,
the delivery sprint and ``compute_dq``.
"""
