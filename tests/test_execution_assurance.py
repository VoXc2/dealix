...
def _invoice_payload(lead_id: str, quote_authority_ref: str, *, amount_sar: float = 12345.0) -> dict[str, object]:
    return {
        **_quote_facts(lead_id, amount_sar=amount_sar),
        "quote_authority_ref": quote_authority_ref,
        "idempotency_key": f"test-invoice:{lead_id}:{quote_authority_ref}",
    }


def _request_quote_authority(cli: TestClient, lead_id: str, *, amount_sar: float = 12345.0) -> str:
    response = cli.post("/api/v1/quotes/authority/request", headers=_ADMIN, json=_quote_facts(lead_id, amount_sar=amount_sar))
...
def _approve_quote_authority(cli: TestClient, approval_id: str) -> None:
    response = cli.post(f"/api/v1/approvals/{approval_id}/approve", json={"who": "founder-test"})
    assert response.status_code == 200, response.text
...
def test_support_affiliate_misleading_claim_blocked():
    from dealix.revenue_ops_autopilot.support_pipeline import analyze_support
    s = analyze_support("Dealix تضمن لك نمو الإيراد وتؤتمت كل مبيعاتك تلقائياً بدون تدخل.")
    assert s.intent == "unsupported_financial_claim"
    assert s.kb_auto_allow is False
...
def test_quote_authority_request_does_not_calculate_or_send_price():
    ...
    st.upsert_lead(FunnelLeadRecord(id="lea_quote_request", email="quote@example.com", company="Quote Co", stage="meeting_done"))
    cli = TestClient(app)
...
def test_quote_authority_request_blocked_before_qualified_discovery():
    from api.main import app
    from dealix.revenue_ops_autopilot.store import get_autopilot_store
    get_autopilot_store().upsert_lead(FunnelLeadRecord(id="lea_pre_discovery", email="pre@example.com", company="Pre", stage="qualified_A"))
    r = TestClient(app).post("/api/v1/quotes/authority/request", headers=_ADMIN, json=_quote_facts("lea_pre_discovery"))
...
def test_invoice_draft_blocked_before_scope_sent_api():
    from api.main import app
    from dealix.revenue_ops_autopilot.store import get_autopilot_store
    get_autopilot_store().upsert_lead(FunnelLeadRecord(id="lea_pre_scope", email="founder@example.com", company="Acme", stage="qualified_A"))
    r = TestClient(app).post("/api/v1/invoices/draft", headers=_ADMIN, json=_invoice_payload("lea_pre_scope", "apr_missing"))
    assert r.status_code == 422, r.text
    assert r.json()["detail"]["reason"] == "invoice_draft_blocked_until_scope_sent"


def test_invoice_draft_rejects_retired_tier_catalog_payload():
    from api.main import app
    from dealix.revenue_ops_autopilot.store import get_autopilot_store
    get_autopilot_store().upsert_lead(FunnelLeadRecord(id="lea_legacy_tier", email="legacy@example.com", company="Legacy", stage="scope_sent"))
    r = TestClient(app).post("/api/v1/invoices/draft", headers=_ADMIN, json={"lead_id": "lea_legacy_tier", "tier": "starter"})
    assert r.status_code == 422, r.text
...
def test_invoice_draft_rejects_missing_quote_authority():
    from api.main import app
    from dealix.revenue_ops_autopilot.store import get_autopilot_store
    get_autopilot_store().upsert_lead(FunnelLeadRecord(id="lea_missing_auth", stage="scope_sent"))
    r = TestClient(app).post("/api/v1/invoices/draft", headers=_ADMIN, json=_invoice_payload("lea_missing_auth", "apr_does_not_exist"))
    assert r.status_code == 422, r.text
...
def test_invoice_draft_rejects_pending_quote_authority():
    from api.main import app
    from dealix.revenue_ops_autopilot.store import get_autopilot_store
    get_autopilot_store().upsert_lead(FunnelLeadRecord(id="lea_pending_auth", stage="scope_sent"))
    cli = TestClient(app)
    approval_id = _request_quote_authority(cli, "lea_pending_auth")
    r = cli.post("/api/v1/invoices/draft", headers=_ADMIN, json=_invoice_payload("lea_pending_auth", approval_id))
    assert r.status_code == 422, r.text
    body = r.json()["detail"]
...
def test_invoice_draft_rejects_amount_tampering_after_quote_approval():
    from api.main import app
    from dealix.revenue_ops_autopilot.store import get_autopilot_store
    get_autopilot_store().upsert_lead(FunnelLeadRecord(id="lea_tamper", stage="scope_sent"))
    cli = TestClient(app)
    approval_id = _request_quote_authority(cli, "lea_tamper", amount_sar=12345.0)
    _approve_quote_authority(cli, approval_id)
    r = cli.post("/api/v1/invoices/draft", headers=_ADMIN, json=_invoice_payload("lea_tamper", approval_id, amount_sar=12000.0))
    assert r.status_code == 422, r.text
...
def test_invoice_draft_ok_when_scope_sent_and_quote_authorized():
    from api.main import app
    from dealix.revenue_ops_autopilot.store import get_autopilot_store
    get_autopilot_store().upsert_lead(FunnelLeadRecord(id="lea_scoped", email="ops@example.com", company="Beta", stage="scope_sent"))
    cli = TestClient(app)
    approval_id = _request_quote_authority(cli, "lea_scoped")
    _approve_quote_authority(cli, approval_id)
    r = cli.post("/api/v1/invoices/draft", headers=_ADMIN, json=_invoice_payload("lea_scoped", approval_id))
...
def test_full_ops_health_endpoint():
    from api.main import app
    r = TestClient(app).get("/api/v1/ops-autopilot/full-ops-health", headers=_ADMIN)
    assert r.status_code == 200, r.text
    data = r.json()
...
def test_compute_full_ops_health_evidence_completeness():
    ...
    st.append_evidence(EvidenceEvent(id=uid("ev"), event_type="lead_captured", entity_type="funnel_lead", entity_id="l1", summary="seed"))
