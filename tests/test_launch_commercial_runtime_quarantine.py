...
_BLOCKED_RUNTIME_PATHS = {
    "/api/v1/pricing/plans", "/api/v1/pricing/usage", "/api/v1/pricing/menu",
    "/api/v1/checkout", "/api/v1/pricing/outcome-simulate", "/api/v1/billing/plans",
    "/api/v1/billing/subscribe", "/api/v1/billing/upgrade", "/api/v1/billing/cancel",
    "/api/v1/billing/invoices/{invoice_id}/pay", "/api/v1/finance/pricing",
    "/api/v1/finance/pricing/{tier_id}", "/api/v1/finance/invoice/draft",
    "/api/v1/onboarding/plans", "/api/v1/onboarding/signup", "/api/v1/payments/plans",
    "/api/v1/payments/subscriptions/create", "/api/v1/payments/moyasar/charge",
    "/api/v1/payments/stc-pay/create", "/api/v1/payments/sadad/create-bill",
    "/api/v1/command-center/agents", "/api/v1/command-center/agents/{agent_id}",
    "/api/v1/command-center/leaks", "/api/v1/command-center/proof-pack",
}

_REQUIRED_SAFE_PATHS = {
    "/api/v1/public/services", "/api/v1/commercial-map", "/api/v1/commercial-map/markdown",
    "/api/v1/ops-autopilot/leads/{lead_id}/meeting-brief", "/api/v1/quotes/authority/request",
    "/api/v1/invoices/draft", "/api/v1/webhooks/moyasar", "/api/v1/billing/subscription",
    "/api/v1/billing/invoices", "/api/v1/billing/features", "/api/v1/finance/status",
    "/api/v1/business/pricing", "/api/v1/commercial/status", "/api/v1/services/status",
    "/api/v1/services/catalog", "/api/v1/onboarding/wizard", "/api/v1/onboarding/invite",
    "/api/v1/sector-intel/sectors", "/api/v1/service-setup/requests",
}


def _collect_paths(routes: object, parent_prefix: str = "") -> list[str]:
    """Collect effective paths across flat and FastAPI preserved router trees."""
    paths: list[str] = []
    for route in routes:  # type: ignore[union-attr]
        if type(route).__name__ == "_IncludedRouter":
            ...
            if original_router is not None:
                paths.extend(_collect_paths(original_router.routes, parent_prefix + prefix))
            continue
...
def _mounted_paths() -> list[str]:
    return _collect_paths(create_app().routes)
...
def test_roi_summary_requires_customer_specific_quote_amount() -> None:
    ...
    payload = asyncio.run(proof_pack_roi({"subscription_sar": 1234.5, "influenced_revenue_sar": 100000, "hours_saved": 10}))
...
def test_safe_commercial_map_keeps_compatibility_url_without_price_or_checkout() -> None:
    ...
    rendered = repr(data)
    for retired in ("seven_day_governance_diagnostic", "2999", "4999", "7999", "9999", "15000", "/api/v1/checkout"):
...
def test_public_service_catalog_exposes_only_launch_authorized_offers_without_prices() -> None:
    ...
    assert r.status_code == 200, r.text
    data = r.json()
    assert [row["id"] for row in data["offerings"]] == ["free_mini_diagnostic", "revenue_command_pilot_30d"]
    assert data["count"] == 2
...
def test_bespoke_service_intake_never_generates_automatic_price() -> None:
    cli = TestClient(create_app())
    r = cli.post("/api/v1/service-setup/requests", json={
        "company_name": "Acme Saudi", "contact_name": "Owner", "contact_email": "owner@example.com",
        "use_case_summary": "We need a governed revenue workflow with source-bound evidence.",
        "use_case_category": "sales", "complexity": "moderate", "integrations_count": 2,
        "data_volume_band": "medium", "timeline_weeks": 4, "regulated_industry": False,
    })
