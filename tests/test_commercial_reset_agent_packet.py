...
def test_daily_packets_use_resource_governed_holding_dispatch() -> None:
    ...
    assert all(row["agent_parent"].startswith("dealix.") for row in payload["sector_company_packets"])
...
def test_legacy_sales_alias_resolves_to_group_revenue_role() -> None:
    ...
    assert packet["price_authority"] == "customer-specific / approval-bound"
    assert packet["problem_evidence"] == ["source-1"]
    assert "research != relationship" in packet["truth_firewall"]


def test_sector_diagnostic_packet_gets_sector_parent() -> None:
    packet = build_work_packet(
        layer="sector",
        function="DIAGNOSTIC",
    ...
    assert packet["agent_identity"] == "dealix.construction-contractors.sector-diagnostic"
    assert packet["agent_parent"] == "dealix.construction-contractors"
    assert packet["agent_layer"] == "sector"
    assert packet["agent_role"] == "sector-diagnostic"


def test_arm_proof_packet_gets_arm_parent() -> None:
    packet = build_work_packet(
        layer="arm",
        function="PROOF",
        opportunity_id="OPP-003",
        entity="Retail Co",
        sector="wholesale-retail-ecommerce",
        arm="arm_42_proof_ledger",
        stage="DELIVERY",
        problem="Need source-bound outcome evidence",
    )
    assert packet["agent_identity"] == (
        "dealix.wholesale-retail-ecommerce.arm_42_proof_ledger.verifier"
    )
    assert packet["agent_parent"] == (
        "dealix.wholesale-retail-ecommerce.arm_42_proof_ledger"
    )
