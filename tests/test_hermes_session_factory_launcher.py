...
RENDERER = ROOT / "scripts/ops/render_hermes_session_factory_launcher.py"
VERIFIER = ROOT / "scripts/ops/verify_hermes_session_factory_launcher.py"
INSTALLER = ROOT / "scripts/ops/install_hermes_session_factory_launcher.sh"


def _load_renderer():
    spec = importlib.util.spec_from_file_location("hermes_launcher_renderer", RENDERER)
...
def test_rendered_launcher_is_canonical_and_model_neutral(tmp_path: Path) -> None:
    renderer = _load_renderer()
    sha = "b" * 40
    repo = tmp_path / "repo"
    text = renderer.render_launcher(sha, repo=repo, state_dir=tmp_path / "state")
    assert f"EXPECTED_SHA={sha}" in text
    assert f"REPO={repo}" in text
    assert "/opt/dealix/control/runtime/session-factory-" not in text
...
def test_source_identity_mismatch_holds_before_runtime(tmp_path: Path) -> None:
    ...
    launcher.write_text(
        renderer.render_launcher(
            ...
            lock_file=tmp_path / "factory.lock",
        ),
        encoding="utf-8",
    )
