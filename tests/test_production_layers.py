...
from dealix.commercial_ops.selfhost_production import check_repo_selfhost_config


def test_repo_selfhost_config_ok() -> None:
    repo = check_repo_selfhost_config()
    assert repo["ok"], repo["issues"]


def test_layer_1_repo_without_env() -> None:
    layer = layer_1_selfhost_p0(check_env=False)
    assert layer["pct"] >= 50
