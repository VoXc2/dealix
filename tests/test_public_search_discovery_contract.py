...
AGENTS_PAGE = ROOT / "apps/web/app/agents/page.tsx"
...
def test_chatgpt_search_crawler_is_allowed_without_enabling_training_bot() -> None:
    ...
    for private_path in ("/control-plane", "/approvals", "/sandbox", "/self-evolving", "/api/", "/healthz"):
        assert private_path in robots
    # /agents is a public retirement redirect; crawlers must be able to see the redirect.
    assert '"/agents"' not in robots
...
def test_legacy_search_and_demo_agent_routes_retire_to_canonical_public_surfaces() -> None:
    ...
    agents = AGENTS_PAGE.read_text(encoding="utf-8")
    ...
    assert 'permanentRedirect("/dealix-os")' in agents
