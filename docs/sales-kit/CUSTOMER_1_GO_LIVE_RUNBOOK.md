...
5. شغّل بدء التسليم:
   ...
   احفظ هذا الـ`delivery_kickoff_id` — هو **رابط التدقيق** بين الدفعة
   والتسليم، وستستخدمه كـ`engagement_id` للـSprint.
...
6. شغّل المُنسِّق ذا الـ10 خطوات ببيانات العميل الحقيقية:
   ...
   ```
   → النتيجة تتضمن Proof Pack مُقيَّماً (14 قسماً) + capital assets + أهلية
   الـretainer.

## 4. تسليم Proof Pack مُنسَّق | Render the deliverable

7. ولّد الـProof Pack بصيغة PDF (أو Markdown احتياطياً):
   `POST /api/v1/sprint/render/pdf` (نفس body الخطوة 6) → ملف PDF.
   إن لم يتوفّر مُحرِّك PDF يرجع Markdown مع ترويسة
   `X-PDF-Renderer: unavailable`.
8. ولّد نص بريد جاهز للإرسال:
   `POST /api/v1/sprint/render/email-body` → رسالة غلاف ثنائية اللغة.
...
> `POST /api/v1/diagnostic/report/pdf` و `/report/markdown`.
