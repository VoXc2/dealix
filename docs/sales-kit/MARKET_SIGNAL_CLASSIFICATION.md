...
|---|---|---|---|---|
| `replied_interested` | رد بالاهتمام | Reply within 30 min (working hours), never sooner than 1h after their message. Request a 20-min qualification call. | رد خلال 30 دقيقة، اطلب مكالمة تأهيل 20 دقيقة. | War Room §3 |
...
| `qualify_decision_returned` | عاد قرار التأهيل | Log the five-decision outcome (`ACCEPT` / `DIAGNOSTIC_ONLY` / `REFRAME` / `REJECT` / `REFER_OUT`) and follow `WARM_LIST_WORKFLOW.md` §5. | سجّل قرار التأهيل واتبع §5. | DECISION_LEDGER |
...
| `no_response_after_follow_up` | لا رد بعد متابعة | Mark `no_response_after_follow_up` and move on. One follow-up was the maximum — no chasing. Re-engage only on a natural occasion. | علّمها وانتقل — لا مطاردة. متابعة واحدة كانت الحد الأقصى. | War Room §3 + FRICTION_LOG |
