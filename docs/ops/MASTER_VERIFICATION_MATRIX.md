...
| 28 | Brand Architecture | `docs/brand/BRAND_ARCHITECTURE.md`, `docs/brand/MASTER_BRAND.md`, `docs/brand/SUB_BRAND_RULES.md`, `partner-kit/branding/HOLDING_BRAND_NOTE.md` | `scripts/render_holding_portfolio.py` | `tests/test_brand_architecture_lists_all_active_units.py`, `tests/test_brand_architecture_doctrine_endorsement.py` | `landing/group.html`, `landing/assets/data/holding-portfolio.json` | Group landing | Multi-brand discipline | **PASS** |
| 29 | M&A & BU Lifecycle Discipline | `docs/holding/ACQUISITION_THESIS.md`, `docs/holding/MA_PLAYBOOK.md`, `docs/holding/SUBSIDIARY_ONBOARDING.md`, `docs/holding/BU_KILL_RULES.md` | `scripts/draft_bu_decision_memo.py` | `tests/test_bu_kill_rules_match_unit_governance.py`, `tests/test_ma_playbook_lists_required_artifacts.py` | `data/_state/bu_memos/<slug>_<YYYY-MM>.md` (cron) | Board-memo card | Disciplined growth + kill | **PASS** |
| 30 | Holding Investor Surface | `docs/holding/{GROUP_RISK_REGISTER,HOLDING_COMPLIANCE,INTERNAL_AUDIT,CAP_TABLE,BOARD_OF_DIRECTORS,BOARD_CHARTER,HOLDING_INVESTMENT_MEMO}.md` | `api/routers/cap_table.py` | `tests/test_cap_table_public_is_amount_safe.py`, `tests/test_cap_table_ratios_sum_to_100.py`, `tests/test_group_risk_register_completeness.py`, `tests/test_holding_landing_renders.py` | `data/{cap_table,group_risk_register,board_of_directors}.json`, `landing/{holding,annual-report,portfolio}.html`, `GET /api/v1/holding/cap-table{,/public}` | Holding landing | Investor / lender / auditor diligence | **PASS** |
